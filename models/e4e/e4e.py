...
class SingleE4e(Module):
    def __init__(self, stylegan_size=256, pretrained=True):
        super().__init__()
        num_layers = 50
        blocks = get_blocks((num_layers))
        unit_module = bottleneck_IR_SE
    ...
    def load(self):
        state_file = os.path.join(thisdir, 'outdir_sse/checkpoints/best_model.pt')
        ...
        self.register_buffer('latent_avg', torch.zeros(1, self.style_count, 512))
...
class E4e(Module):
    ...
    def load(self):
        state_dict = torch.load('checkpoints/encoder_state_dict.pt', map_location='cpu')
        state_dict['latent_avg'] = torch.load('checkpoints/latent_avg.pt', map_location='cpu').unsqueeze(0)
        self.load_state_dict(state_dict)
